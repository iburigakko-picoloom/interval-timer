...
export function setDuration(input, seconds) {
  ...
  fields[0].value = Math.floor(seconds / 60);
  fields[1].value = seconds % 60;
...
export function enhanceDuration(input, name, maxSeconds) {
  ...
  for (const [unit, factor, max] of [['分', 60, Math.floor(maxSeconds / 60)], ['秒', 1, 59]]) {
  ...
  const validate = () => {
    ...
    input.setCustomValidity(total < 1 || total > maxSeconds ? `1秒から${Math.floor(maxSeconds / 60)}分の範囲で入力してください` : '');
  };
